...
- **All ~496 NYC subway stations searchable, correctly grouped and sorted** — bundles MTA's published station list (`assets/data/mta_stations.csv`) as a local asset; tapping a station shows live arrivals split into uptown/downtown (or the station's actual direction labels) tabs. Verified on an Android emulator against the real feed for multiple stations. Stations sort in natural numeric order (1, 2, 3…96, 103, 110, not string order), and station names that map to more than one physical station (e.g. multiple "Canal St"s, some connected, some not) show as a single grouped row with a picker on tap rather than duplicate rows.
