...
- **Phase 2 backend scaffolded** (`backend/`) — FastAPI + SQLAlchemy 2.0 + Alembic, matching the PRD's `users`/`trips`/`preferences` schema. Since Phase 2's user model decision landed on a real multi-user product (not a personal tool), `users` includes proper auth: email/password signup and login issuing JWT access tokens, a `/users/me` endpoint demonstrating the auth dependency, plus `POST /trips` for logging trip data points. Every signup gets a `preferences` row created at PRD-specified defaults immediately, ready for the (not yet built) nightly batch job to update. 12 passing tests (SQLite in-memory, not Postgres — no Postgres-specific features are exercised yet). PostgreSQL itself isn't installed on the dev machine yet (see `OPEN_QUESTIONS.md`) so the Alembic migration is hand-written and syntax-checked but not yet run against a live database.
- **Mobile app now talks to the backend** (`lib/account/`) — an optional account screen (signup/login/logout), reachable from the favorites screen's app bar; browsing and favoriting still never require an account, per an explicit product decision. Logging in turns on passive trip logging: opening any station's arrivals screen posts a trip data point (station, agency, timestamp) to the backend for logged-in users only — this is the raw signal Phase 2's future preference-learning job will read. `dest_stop` is intentionally left null for now since the app doesn't yet capture a real destination (no route-planning feature exists), rather than guessing or duplicating the origin. Points at the dev machine's local network IP so a phone on the same Wi-Fi can reach it during development — will need a real hosted URL once backend hosting is decided (still an open question).
